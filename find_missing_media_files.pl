...
while(my @row = $sth->fetchrow_array()) {
   ...
   # if ($row[0] eq 350 || $row[0] eq 493 || $row[0] eq 7418) {
   #    next;
   # }
      if ($MediaID eq 308) {
   ...
   unless ( -r "$relpath" ) {
      ...
      if ( -r "$try1" ) {
         ...
         # $stmt = qq(UPDATE MultimediaTable SET Thumbnail=NULL WHERE MediaID = $MediaID ;);
         # printf "%s\n", $stmt;
         ...
         if ($MediaType ne 2) {
            ...
            $stmt = qq(UPDATE MultimediaTable SET MediaFile=NULL WHERE MediaID=$MediaID ;);
         }
         printf "%s\n", $stmt;
         $sth = $dbh->prepare( $stmt );
